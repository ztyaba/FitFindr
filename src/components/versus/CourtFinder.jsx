...
export default function CourtFinder({ courts = [], onHover, onHoverExit, onCourtClick, variant = "default" }) {
    ...
    <div className={isCompact ? "space-y-4" : "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8"}>
      {courts.map((court, index) => {
        ...
        return (
          ...
          >
            <Card className={`h-full bg-slate-900/40 backdrop-blur-3xl border border-white/10 ${isCompact ? 'rounded-2xl' : 'rounded-[2.5rem]'} overflow-hidden group hover:bg-slate-900/60 transition-all duration-500 shadow-2xl`}>
              {!isCompact && (
              ...
              <CardHeader className={isCompact ? "p-4 pb-2" : "p-8 pb-4"}>
                <div className="flex justify-between items-start gap-4 mb-1">
                  <CardTitle className={`${isCompact ? 'text-lg' : 'text-2xl'} font-black text-white tracking-tight group-hover:text-blue-400 transition-colors line-clamp-1 leading-tight`}>
                    {court.name}
                  </CardTitle>
                  {isCompact && (
                    <div className="flex items-center gap-1 text-[10px] font-black text-white">
                      <Star className="w-3 h-3 fill-yellow-500 text-yellow-500" />
                      {court.rating?.toFixed(1) || "4.8"}
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                  ...
                  {court.location?.city || "Unknown"}
                </div>
              </CardHeader>

              <CardContent className={isCompact ? "p-4 pt-0" : "p-8 pt-0 space-y-8"}>
                {!isCompact && (
                  <>
                    <div>
                    ...
                    <div className="flex gap-3 pt-2">
                      {court.contact_info?.phone && (
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleContact("phone", court.contact_info.phone);
                          }}
                      ...
                      {court.contact_info?.website && (
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleContact("website", court.contact_info.website);
                          }}
                  ...
                  <div className="mt-4 flex flex-col gap-3 pt-3 border-t border-white/5">
                    <div className="flex flex-wrap gap-1.5">
                      {supportedSports.slice(0, 3).map((sport) => (
                        <Badge
                          key={sport}
                          className={`px-2 py-0.5 rounded-lg text-[8px] font-black uppercase tracking-widest border ${SPORT_COLORS[sport] || "bg-slate-500/10 text-slate-400 border-slate-500/20"}`}
                        >
                          {sport.replace(/_/g, " ")}
                        </Badge>
                      ))}
                      {supportedSports.length > 3 && (
                        <span className="text-[8px] font-black text-slate-500 uppercase flex items-center">
                          +{supportedSports.length - 3}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <div className="text-sm font-black text-white">${court.hourly_rate || court.rate_per_hour}</div>
                        <div className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">/ HR</div>
                      </div>
                      <div className="text-[9px] font-black text-blue-400 uppercase tracking-widest bg-blue-400/10 px-2 py-0.5 rounded-md border border-blue-400/20">
                        Top Rated
                      </div>
                    </div>
                  </div>
